...
JSON-LD is a lightweight syntax to encode Linked Data in JSON. Its design allows existing JSON to be interpreted as Linked Data with minimal changes. JSON-LD is 100% compatible with JSON. JSON-LD introduces a universal identifier mechanism for JSON objects via the use of a Uniform Resource Identifier (URI) and a way to associate data types with property values.
...
The authors of this document perceive that the current documentation of JSON-LD as confusing. providing much information that is difficult to understand from the point of view of a JSON and XML user. That is why another approach in explaining how to use JSON-LD is presented here.
...
Within the context of RDF, a triple is a construct built from a subject, predicate and an object. The conversion to RDF results in a single triple stating that the Mississippi river is a river.
...
To add a property to the Mississippi river we should define the semantics of the property by associating a URI to the property name. To do this we need to also add a @context property. Here we have two possibilities:
...
----
{
    "@context": ["http://schema.org/", {
        ...
        "dbresource": "http://dbpedia.org/resource/",
        "dbpedia": "http://dbpedia.org/ontology/",
        "bridges": "owscommon:river/bridge"
    }],
    "@id": "dbresource:Mississippi_River",
    "@type": "dbpedia:River",
    "name": "Mississippi river",
    "bridges": ["Eads Bridge", "Chain of Rocks Bridge"]
}
